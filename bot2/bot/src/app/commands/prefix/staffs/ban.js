 ...
 */
class Ban extends PrefixCommands {
    constructor() {
        super({
            name: "ban",
            alias: ["banear"],
    ...
    async run(client, message, args) {
        ...
        if (!member) return message.reply("Por favor, menciona al usuario que deseas banear.");
        ...
        member.ban({ reason })
            .then(async () => {
                message.channel.send(`${member.user.tag} fue baneado. Razón: ${reason}`);
                ...
                channel.send(`El usuario ${member.user.tag} fue baneado por ${message.author.tag}. Razón: ${reason}`);
            })
