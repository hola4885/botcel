...
/**
 * Clase para representar el comando addticket.
 * @extends PrefixCommands
 */
class AddTicket extends PrefixCommands {
    constructor() {
        super({
            name: "addticket"
        });
    ...
    async run(client, message, args) {
        try {
            const staffRoleId = "1328840022840971314";  // Reemplaza con el ID del rol Staff
            ...
            if (!user) return message.channel.send("Debes mencionar un usuario válido. Uso: `zzp!addticket @usuario`");

            // Otorgar permisos al usuario mencionado
            ...
            const embed = new EmbedBuilder()
                .setDescription(`${user} ahora tiene acceso a este ticket.`)
                .setColor("#4D00FF");
